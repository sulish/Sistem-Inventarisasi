...
?>
		...
		<h2>DATA REKAP PENGEMBALIAN ALAT</h2>
...
<?php
	// ALAT
		...
		where detail_pa.status='2' and transaksi_pa.indate like '%$_POST[cari]%' or transaksi_pa.outdate like '%$_POST[cari]%' or laboran.nama like '%$_POST[cari]%'
		or alat.nama like '%$_POST[cari]%' order by transaksi_pa.indate asc");
}else{
  		$query=mysql_query("SELECT
		transaksi_pa.id_pinjam,
		transaksi_pa.indate,
		...
		INNER JOIN alat ON detail_pa.id_alat = alat.kd_barang
		INNER JOIN `user` ON transaksi_pa.id_user = `user`.id_user
		where detail_pa.status='2' and transaksi_pa.id_lab='$_SESSION[id]'
